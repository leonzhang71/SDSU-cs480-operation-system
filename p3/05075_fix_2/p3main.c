 ...
 * Other side effects:
   ...
   By contrast, Linux stores these named files under /dev/shm/ .
...
int main(int argc, char *argv[]) {
    ...
    quota = (argc>3) ? atoi(argv[3]) : QUOTA;
    ...
    if (SEM_FAILED == done){
        fprintf(stderr,"Semaphore initialization failed in p3main.c -- exiting\n");
        exit(3);
    }
