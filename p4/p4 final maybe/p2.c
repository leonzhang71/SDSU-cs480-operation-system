    ...
    for (i=1; i<cmds[0].num_args; i++) {
        ...
        while(dirp) {
            if ((dp = readdir(dirp)) != NULL) {
               ...
               snprintf(path,1023,"%s/%s",cmds[0].cmd_args[i],dp->d_name); 
...
/* Declaration of Local*/
    ...
    if (pid == 0) {
        ...
        int in_fd;
        int out_fd;
        if (input_file || background_flag) {
            const char* fpath = input_file;
            if (fpath == NULL) {
                fpath = "/dev/null";
            }
            if ((in_fd = open(fpath, O_RDONLY)) < 0) {
                perror("open");
                _exit(-1);
            }
        ...
        if (output_file) {
            if ((out_fd = open(output_file, O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH)) < 0) {
                // perror("open");
...
static int run_other_cmds(int i, int fds[])
{
    if (i == 0) {
        // first cmd
...
int run_piped_cmds()
{
    ...
    if (pid == 0) {
        ...
        if (pid2 == 0) {
            // grandson
            run_other_cmds(num_cmds-2, fds);
            _exit(1);
        }


        if (output_file) {
            int out_fd = open(output_file, O_WRONLY|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
            if (out_fd < 0) {
                fprintf(stderr, "open %s failed\n", output_file);
                _exit(1);
            }
        ...
        index = num_cmds - 1;
        if (execvp(cmds[index].cmd_args[0], cmds[index].cmd_args) < 0) {
            close(STDIN_FILENO);
            perror("execvp error");
            for (;;) {
                pid_t tpid = wait(NULL);
